...
struct lock {
	...
	Drawable drawable;
	GC gc;
	XPoint triangles[LENGTH(triangles)][NUM_POINTS_POLYGON];
	XPoint innertriangles[LENGTH(triangles)][NUM_POINTS_POLYGON];
};
...
static void
resizetriangles(struct lock *lock)
{
	for ( int i = 0; i < LENGTH(triangles); i++ ){
		for ( int j = 0; j < LENGTH(triangles[i]); j++ ) {
			lock->triangles[i][j].x = (triangles[i][j].x *(float) logosize)
                        	        + ((lock->mw) / 2);
			lock->triangles[i][j].y = (triangles[i][j].y *(float) logosize)
                        	        + ((lock->mh) / 2);
		}
	}
}
...
static void
resizeinnertriangles(struct lock *lock)
{
	for ( int i = 0; i < LENGTH(triangles); i++ ){
		float xpointoffset = 0;
		float ypointoffset = 0;
		for (int j = 0; j < LENGTH(triangles[i]); j++) {
			xpointoffset += triangles[i][j].x;
			ypointoffset += triangles[i][j].y;
		}
		xpointoffset /= LENGTH(triangles[i]);
		ypointoffset /= LENGTH(triangles[i]);

		for ( int j = 0; j < LENGTH(triangles[i]); j++ ) {
			lock->innertriangles[i][j].x = (triangles[i][j].x *(float) innerlogosize)
                        	        + ((lock->mw) / 2) + (xpointoffset * ((float)(logosize - innerlogosize)));
			lock->innertriangles[i][j].y = (triangles[i][j].y *(float) innerlogosize)
                        	        + ((lock->mh) / 2) + (ypointoffset * ((float)(logosize - innerlogosize)));
		}
	}
}
...
drawlogo(Display *dpy, struct lock *lock, int color)
{
	...
	for ( int i = 0; i < LENGTH(triangles); i++ ) {
		XFillPolygon(dpy, lock->drawable, lock->gc, lock->triangles[i], LENGTH(triangles[i]), Convex, CoordModeOrigin);

	}
	XSetForeground(dpy, lock->gc, lock->colors[color + 3]);
	for ( int i = 0; i < LENGTH(triangles); i++ ) {
		XFillPolygon(dpy, lock->drawable, lock->gc, lock->innertriangles[i], LENGTH(triangles[i]), Convex, CoordModeOrigin);
...
lockscreen(Display *dpy, struct xrandr *rr, int screen)
{
	...
	resizetriangles(lock);
	resizeinnertriangles(lock);
